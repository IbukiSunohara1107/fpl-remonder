...
const CACHE_NAME = 'fpl-reminder-v3';
...
async function fetchEvents() {
  ...
  try {
    const res  = await fetch(FPL_API, { cache: 'no-cache' });
    ...
    console.log(`[SW] Fetched ${events.length} events from API.`);
    return events;
  } catch (err) {
    console.warn('[SW] API fetch failed, trying cache:', err);
    const cached = await cache.match(CACHE_KEYS.EVENTS);
    if (cached) {
      const events = await cached.json();
      console.log(`[SW] Loaded ${events.length} events from cache.`);
      return events;
    }
    return null;
  }
